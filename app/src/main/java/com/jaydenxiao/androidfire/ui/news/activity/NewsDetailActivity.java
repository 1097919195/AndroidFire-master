...
public class NewsDetailActivity extends BaseActivity<NewsDetailPresenter, NewsDetailModel> implements NewsDetailContract.View {
    ...
    public void returnOneNewsData(NewsDetail newsDetail) {
        ...
        setToolBarLayout(mNewsTitle);//标题
        //mNewsDetailTitleTv.setText(newsTitle);
        newsDetailFromTv.setText(getString(R.string.news_from, newsSource, newsTime));
        setNewsDetailPhotoIv(NewsImgSrc);//图片
        setNewsDetailBodyTv(newsDetail, newsBody);//内容
    }
...
//              mNewsDetailBodyTv.setMovementMethod(LinkMovementMethod.getInstance());//加这句才能让里面的超链接生效,实测经常卡机崩溃
            ...
            newsDetailBodyTv.setText(Html.fromHtml(newsBody, mUrlImageGetter, null));//将Html显示在TextView中(http://blog.csdn.net/johnsonblog/article/details/7741972)
        } else {
