...
public class DatasUtil {
     ...
     * 获取发布地址
     */
    public static String getPublishAddresss() {
        String localize = LocalizeUtil.getLngAndLat(AppApplication.getAppContext());//传入参数获取经纬度
        return LocalizeUtil.getLatLongString();
    }

    public static Result getPublishAddress() {
        LocalizeUtil.getLngAndLat(AppApplication.getAppContext());//传入参数获取经纬度
        String addr = "{" + "\"address\":\"" + LocalizeUtil.getLatLongString() + 1+"\"}";
        LogUtils.loge("strrrrrr=========="+addr);
        return JSON.parseObject(addr,Result.class);
    }
