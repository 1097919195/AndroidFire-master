...
public class NewsChannelModel implements NewsChannelContract.Model {
    ...
    public Observable<List<NewsChannelTable>> lodeMineNewsChannels() {
        //创建一个上游 Observable(发送事件,当调用了XXpresenter中的subscribe方法,Observable.OnSubscribe的call方法就会被执行，而Observable.OnSubscribe 说白了就是一个继承了Action1接口的接口，Observable.OnSubscribe <- Action1 <- Action <- Function,create()方法也就是个工厂方法)
